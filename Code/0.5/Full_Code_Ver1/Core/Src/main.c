...
volatile float e2_buffer_array[1500];
volatile uint16_t count_e2 = 0;
	...
	if (htim->Instance == TIM2 && stop == false)
	{
		if (time_ratio == 5)
		{
			if (is_steering == false && recovery_mode == false)
			{
				...
				e2_buffer_array[count_e2] = curr_e2;
				count_e2++;
				...
				if (displacement <= 2900 && curr_e2 >= 26.0){
					recovery_mode = true;
					outline_right = true;
				...
				if (displacement >= 260 && displacement <= 2280){
...
//					calculate_PID_trackingline(v_ref, curr_e2, prev_e2, &delta_w, &pre_Dpart, &w_right, &w_left, &displacement);
				...
				else if (displacement >= 2875 && count_decelartion_2 != 21){
