...
//}
	...
	/* FUNCTION CALCULATE PID CONTROLLER FOR LINE FOLLOWING */
// @brief:	This function calculates output of PID controller for line following task.
//			This function operates at each CONTROL_SAMP_TIME.
// @args:	e2: current error e2.
//			pre_e2: previous error e2.
//			*_delta_w: output of PID controller. Physics mean: angular speed of robot to minimize error e2.
// @return: *_w_right_setpoint: speed of right wheel, be calculated from desired velocity of robot and angular speed of robot.
//			*_w_left_setpoint: speed of left wheel.


void calculate_PID_trackingline(volatile float _e2, volatile float _pre_e2, volatile float* _delta_w, volatile float* _pre_delta_w, volatile float *_w_right_setpoint, volatile float *_w_left_setpoint)
{
	*_delta_w = KP * _e2 + KD * (_e2 - _pre_e2) / CONTROL_SAMP_TIME;
    // Transform block to trans delta_u to desired speed of 2 motor
    *_w_left_setpoint  = (V_REF - (L_WHEEL / 2.0f)* *_delta_w) / (D_WHEEL / 2.0f);
    *_w_right_setpoint = (V_REF + (L_WHEEL / 2.0f)* *_delta_w) / (D_WHEEL / 2.0f);
    return;
}
