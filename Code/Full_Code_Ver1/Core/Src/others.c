...
//void check_to_turn(uint16_t* adc_readout)
//{
//	if((empty == false &&  adc_readout[1] >= 2800 && adc_readout[2] <= 3000 && adc_readout[3] >= 2800))
//	{
//		is_steering = true;
//		has_steered = false;
//	}
...
void check_to_turn(uint16_t* adc_readout)
{
	if((empty == false && adc_readout[0] <= 1000 && adc_readout[1] >= 2000 && adc_readout[2] >= 2500 && adc_readout[3] >= 2000 && adc_readout[4] <= 1000))
	{
		is_steering = true;
		has_steered = false;
	}
...
void stop_for_package(uint16_t* adc_readout, volatile float *_w_right, volatile float *_w_left)
{
	...
	{
		HAL_Delay((uint32_t)(313.0 / V_REF * 1000.0 + 200.0));
		stop = true;
		*_w_right = 0;
		*_w_left = 0;
...
{
	HAL_Delay(20);
	if ((adc_readout[0] <= 500) && (adc_readout[1] <= 500) && (adc_readout[2] <= 500) && (adc_readout[3] <= 500) && (adc_readout[4] <= 500))
	{
		stop = true;
		outline = true;
		*_w_right = 0;
